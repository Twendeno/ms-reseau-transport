...
describe('CoordinatePolygonController', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [{ provide: CoordinatePolygonService, useClass: CoordinatePolygonServiceMock  }],
    }).compile();

    controller = module.get<CoordinatePolygonController>(CoordinatePolygonController);
  });
  ...
  describe('create', () => {
    it('should return a coordinatePolygon',  () => {
      expect(controller.create(coordinatePolygonModelMock[0])).resolves.toEqual({message: "coordinatePolygon created"});
    });
  });

  describe('update', () => {
    it('should return a coordinatePolygon',  () => {
      expect(controller.update("1",coordinatePolygonModelMock[0])).resolves.toEqual({message: "coordinatePolygon updated"});
    });
  });

  describe('delete', () => {
    it('should return a coordinatePolygon',  () => {
      expect(controller.delete("1")).resolves.toEqual({message: "coordinatePolygon deleted"});
    });
  });
