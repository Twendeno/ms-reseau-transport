export class RandomValueGenerator {
  private static ALPHANUMERIC_CHARS:string = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
  static generateRandomAlphaNumeric(generatorLength: number): string {
    let result:string = '';
    const charactersLength:number = RandomValueGenerator.ALPHANUMERIC_CHARS.length;
