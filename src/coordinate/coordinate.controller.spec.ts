...
describe("CoordinateController", () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [{ provide: CoordinateService, useClass: CoordinateServiceMock }]
    }).compile();
  ...
  describe("findOne", () => {
    it("should return a coordinate", () => {
      const uuid = "1";
      const coordinate = coordinateModelMock.find(coordinate => coordinate.uuid === uuid);
  ...
  describe("create", () => {
    it("should return a {message: \"Coordinate created\"}", () => {
      expect(controller.create(coordinateModelMock[0])).resolves.toEqual({ message: "Coordinate created" });
    });
  });
  ...
  describe("update", () => {
    it("should return a {message: \"Coordinate updated\"}", () => {
      const uuid = "1";
      expect(controller.update(uuid,coordinateModelMock[0])).resolves.toEqual({ message: "Coordinate updated" });
    });
  });
  ...
  describe("delete", () => {
    it("should return a {message: \"Coordinate deleted\"}", () => {
      const uuid = "1";
      expect(controller.delete(uuid)).resolves.toEqual({ message: "Coordinate deleted" });
    });
  });
