...
describe('DepartmentController', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [{provide: DepartmentService, useClass: DepartmentServiceMock}],
    }).compile();
  ...
  describe('create', () => {
    it('should return a department',  () => {
      expect(controller.create(departmentModelMock[0])).resolves.toEqual({ message: "department created" });
    });
  });

  describe('update', () => {
    it('should return a department',  () => {
      expect(controller.update("1", departmentModelMock[0])).resolves.toEqual({ message: "department updated" });
    });
  });

  describe('delete', () => {
    it('should return a department',  () => {
      expect(controller.delete("1")).resolves.toEqual({ message: "department deleted" });
    });
  });
