...
export class GeometryServiceMock {
  ...
  findOne = jest.fn().mockImplementation((uuid: string) => {
    return Promise.resolve(geometryModelMock.find(geometry => geometry.uuid === uuid));
  });
  ...
  findGeometryByReference = jest.fn().mockImplementation((ref: string) => {
    return Promise.resolve(geometryModelMock.find(geometry => geometry.reference === ref));
  });
